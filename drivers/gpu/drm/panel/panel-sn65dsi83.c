		...
		struct device_node *disp_dsi, struct device_node *np)
{
	...
	for (i = 0; i < ARRAY_SIZE(sn->gp_en); i++) {
		gp_en = devm_fwnode_gpiod_get_index(dev, child, "enable", i,
						    GPIOD_OUT_LOW, "sn65_en");
		if (IS_ERR(gp_en)) {
			ret = PTR_ERR(gp_en);
			if (i)
				break;
			dev_dbg(sn->dev,
				"%s:devm_fwnode_gpiod_get_index enable %d\n",
				__func__, ret);
	...
	for (i = 0; i < 2; i++) {
		irq = of_irq_get(np, i);
		if (irq > 0) {
			gp_irq = devm_fwnode_gpiod_get_index(dev, child, "interrupts",
							     i, GPIOD_IN, "sn65_irq");
			if (!IS_ERR(gp_en)) {
